...
use cadrion_lang::{evaluate, EvalOptions, FeatureIr};
use serde_json::json;
...
fn load_ir(
        ...
        .is_some_and(|e| e == "json")
    {
        ...
        let ir: FeatureIr = serde_json::from_str(&text).map_err(|e| {
            (
                ExitCode::Eval,
                json!({"ok": false, "diagnostics": [{"code": "CADRION-E-EVAL", "message": format!("bad IR json: {e}")}]}),
            )
        })?;
        return Ok(ir);
    }
    ...
    match cli.kernel {
        ...
        | crate::cli::KernelId::TruckBrep => topology_from_ir(ir)
            ...
            .map_err(|m| {
                (
                    ExitCode::Internal,
                    json!({"ok": false, "diagnostics": [{"code": "CADRION-E-TOPO", "message": m}]}),
                )
            }),
...
fn align_cmd(cli: &Cli, a: &crate::cli::AlignArgs) -> ExitCode {
    ...
    match align_refs(&snap, &a.a, &a.b, expect, a.distance, a.tol, a.tol_deg) {
        ...
        Err(e) => {
            emit(
                cli.json,
                &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-ALIGN","message": e.to_string()}]}),
                false,
            );
            ExitCode::Validation
        }
...
fn frame_cmd(cli: &Cli, a: &crate::cli::FrameArgs) -> ExitCode {
    ...
    match frame_of(&snap, &a.selector) {
        ...
        Err(e) => {
            emit(
                cli.json,
                &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-FRAME","message": e.to_string()}]}),
                false,
            );
            ExitCode::Validation
        }
