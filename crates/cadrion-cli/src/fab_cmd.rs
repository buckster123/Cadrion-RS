...
fn fab_dxf_face(cli: &Cli, a: &FabDxfFaceArgs) -> ExitCode {
    ...
    let snap = match cli.kernel {
        crate::cli::KernelId::Occt => {
                ...
                match open_kernel(cli.kernel) {
                    Ok(mut kb) => match execute_ir(kb.as_mut(), &ir) {
                            ...
                            crate::kernel_pick::KernelBox::Occt(k) => {
                                match k.topology_snapshot(sid) {
                                    ...
                                    Err(e) => {
                                        emit(
                                            cli.json,
                                            &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-TOPO","message": e.to_string()}]}),
                                            false,
                                        );
                        ...
                        Err(e) => {
                            emit(
                                cli.json,
                                &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-KERNEL","message": e.to_string()}]}),
                                false,
                            );
                            return ExitCode::Kernel;
                        }
            ...
            #[cfg(not(feature = "occt"))]
            {
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-KERNEL-UNAVAILABLE","message":"occt not in binary"}]}),
                    false,
                );
                return ExitCode::Usage;
            }
        ...
        | crate::cli::KernelId::TruckBrep => match topology_from_ir(&ir) {
            ...
            Err(e) => {
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-TOPO","message": e}]}),
                    false,
                );
    ...
    let pick = if let Some(f) = &a.face {
        ...
        match parse_vec3(n) {
            ...
            None => {
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-USAGE","message":"bad --normal x,y,z"}]}),
                    false,
                );
                return ExitCode::Usage;
            }
    ...
    let result = match face_to_dxf(&snap, &face_sels, &pick, a.plane_tol, 0.15) {
        ...
        Err(e) => {
            emit(
                cli.json,
                &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-DXF-FACE","message": e}]}),
                false,
            );
    ...
    let profile = if let Some(p) = &a.profile_file {
        ...
        match resolve_bundled_profile(&a.profile) {
            ...
            None => {
                ...
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-USAGE","message": format!("unknown profile '{}' (bundled: {}; or --profile-file)", a.profile, ids.join(", "))}]}),
                    false,
                );
                return ExitCode::Usage;
            }
    ...
    let profile = if let Some(ovp) = &a.override_file {
        ...
        let ov = match load_override_json(&text) {
            ...
            Err(e) => {
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-DFM-OVERRIDE","message": e}]}),
                    false,
                );
                return ExitCode::Validation;
            }
        ...
        match apply_override(&profile, &ov) {
            ...
            Err(e) => {
                emit(
                    cli.json,
                    &json!({"ok": false, "diagnostics":[{"code":"CADRION-E-DFM-DRIFT","message": e}]}),
                    false,
                );
                return ExitCode::Validation;
            }
        ...
        match found {
            ...
            None => {
                emit(
                    ...
                    &json!({
                        "ok": false,
                        "diagnostics":[{"code":"CADRION-E-FAB","message":"no slicer found on PATH; install PrusaSlicer/OrcaSlicer or pass --slicer-bin"}],
                        "slicers": slicers,
                    }),
                    false,
                );
                    ...
                    Err(e) => {
                        emit(
                            ...
                            &json!({
                                "ok": false,
                                "error": e.to_string(),
                                "hint": "klipper live start needs curl on PATH (or CADRION_CURL)"
                            }),
                            false,
                        );
                        return ExitCode::Usage;
                    ...
                    Err(e) => {
                        emit(
                            ...
                            &json!({
                                "ok": false,
                                "error": e.to_string(),
                                "hint": "octoprint live start needs curl on PATH (or CADRION_CURL)"
                            }),
                            false,
                        );
                        return ExitCode::Usage;
                    ...
                    Err(e) => {
                        emit(
                            ...
                            &json!({
                                "ok": false,
                                "error": e.to_string(),
                                "hint": "live start needs curl + mosquitto_pub on PATH (or CADRION_CURL / CADRION_MOSQUITTO_PUB)"
                            }),
                            false,
                        );
                        return ExitCode::Usage;
