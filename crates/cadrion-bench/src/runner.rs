...
use cadrion_inspect::{inspect_refs, measure, MeasureKind, MeasureRequest, TopologySnapshot};
use cadrion_kernel::{GeomKernel, MockKernel};
use cadrion_lang::{evaluate, execute_ir, EvalOptions, FeatureIr, IrNode};
...
fn execute_and_topo(
    ir: &FeatureIr,
    kernel: KernelKind,
) -> Result<(cadrion_kernel::ShapeFacts, TopologySnapshot), String> {
    match kernel {
        KernelKind::Mock => {
        ...
        KernelKind::Occt => {
            #[cfg(feature = "occt")]
            {
                let mut k = cadrion_occt::OcctKernel::new();
                let shape = execute_ir(&mut k, ir).map_err(|e| e.to_string())?;
                let facts = k.facts(shape).map_err(|e| e.to_string())?;
            ...
            {
                Err(
                    "occt kernel not compiled into cadrion-bench (rebuild with --features occt)"
                        .into(),
                )
            }
        }
...
fn find_selector(
    report: &cadrion_inspect::RefsReport,
    find: &FindFace,
    normal_tol: f64,
    ...
    let want = cadrion_kernel::Vec3::new(n[0], n[1], n[2]);
