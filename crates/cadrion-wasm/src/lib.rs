//! Cadrion WASM surface (H2-1) — portable IR + mock facts.
//!
//! **Honesty:** mock kernel only; no OCCT; not parity-eligible; not a multi-tenant host.
//! Build: `cargo build -p cadrion-wasm --target wasm32-unknown-unknown --features browser`

#![deny(unsafe_code)]

use std::collections::BTreeMap;

use cadrion_inspect::{box_topology, cylinder_topology, inspect_refs, SolidRec, TopologySnapshot};
use cadrion_kernel::{GeomKernel, MockKernel, Point3};
use cadrion_lang::{evaluate, execute_ir, BooleanKind, EvalOptions, FeatureIr, IrNode};
