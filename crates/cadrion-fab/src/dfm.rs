...
pub const DFM_PROFILE_SCHEMA: &str = "cadrion.dfm_profile";
pub const DFM_OVERRIDE_SCHEMA: &str = "cadrion.dfm_override";
...
pub fn validate_profile(p: &DfmProfile) -> Result<(), String> {
    if !cadrion_kernel::schema_matches(&p.schema, DFM_PROFILE_SCHEMA) {
        return Err(format!(
            "unknown DFM schema '{}' (want {DFM_PROFILE_SCHEMA} or cadre.dfm_profile)",
            p.schema
        ));
    }
...
pub fn load_override_json(text: &str) -> Result<DfmOverride, String> {
    ...
    if !cadrion_kernel::schema_matches(&o.schema, DFM_OVERRIDE_SCHEMA) {
        return Err(format!(
            "unknown override schema '{}' (want {DFM_OVERRIDE_SCHEMA} or cadre.dfm_override)",
            o.schema
        ));
    }
